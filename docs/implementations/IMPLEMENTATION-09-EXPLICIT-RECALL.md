...
补强 128 KiB 验收覆盖：此前“超长中文 transcript”用例中的原文在进入总 bundle fitter 前已先经过 transcript excerpt/context 的局部截断，因此不足以单独证明序列化总预算。现新增两条直接命中总预算的离线回归：初始 bundle 构造真实 `>128 KiB` 的中文 canonical evidence，先断言原始合成 evidence 超限，再验证 UTF-8 安全收缩后 `<=128 KiB` 且原 `source_ref` 不变；expand 则构造同样真实超限的 evidence，并包含一条已由 session 检索注册、但因总字节预算从 fitted bundle 裁掉的来源，验证该 ref 不出现在最终 `source_refs` 且 `deliver_recall` 明确拒绝。该补强只增加验收测试与本报告追加，不改生产逻辑。

补强回归后的代码验收 head `d70b3c05df0387e1e3725e7567d1b2a893fdf1ab` 统一 PR offline CI SUCCESS（run `34022390394`）：51/51 test files、455/455 tests，`recall.test.ts` 22/22，`npm run typecheck` PASS；同一 head 其余 4 个实际专项 checks 全部 success，另 1 项按触发条件正常 skipped。至此 128 KiB 初始/expand 真正超预算、UTF-8 截断、裁掉来源不可 delivery 的补强断言均由干净 GitHub runner 通过。
