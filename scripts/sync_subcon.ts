...
- Each memory_search hit includes quote_snippets containing source-faithful historical excerpts. source_kind=transcript is a direct historical quote; source_kind=legacy_memory is only a fallback when no transcript evidence exists and is an older memory-record excerpt, not a direct quote. If a useful past moment surfaced, call deliver_whisper once with one searched memory_id and 1-3 snippet_ids from that memory. Prefer the fewest quotes that let the moment stand on its own; the runtime renders the selected source excerpts with provenance-appropriate labels.
...
- Never expose memory_search, IDs, snippet IDs, reinforce/remember/create/dedupe, archival status, or maintenance bookkeeping in foreground-visible prose.
