...
async function main(): Promise<void> {
  ...
  try {
    ...
    const enqueuedThroughSnapshot = Math.max(
      state.lastProcessedIndex,
      state.maintenanceEnqueuedThrough ?? state.lastProcessedIndex,
    );
    const candidateNewMessages = formatMessagesForLetta(messages, enqueuedThroughSnapshot, log);

    const spawnDrainer = (): void => {
      const requestFile = path.join(
        TEMP_STATE_DIR,
        `maintenance-drain-${encodeURIComponent(hookInput.session_id)}-${process.pid}-${Date.now()}.json`,
      );
      fs.writeFileSync(requestFile, JSON.stringify({ cwd: hookInput.cwd, sessionId: hookInput.session_id }), { encoding: 'utf8', mode: 0o600 });
      const workerScript = path.join(__dirname, 'maintenance_queue_worker.ts');
      const child = spawnSilentWorker(workerScript, requestFile, hookInput.cwd);
      log(`Poked maintenance queue drainer (PID: ${child.pid})`);
    };

    if (candidateNewMessages.length === 0) {
      if (listMaintenanceQueueJobs(hookInput.cwd, hookInput.session_id).length > 0) {
        log('No unqueued transcript suffix, but maintenance queue is non-empty; poking drainer for retry');
        spawnDrainer();
      } else {
        log('No new messages to enqueue after maintenance watermark');
      }
      process.exit(0);
    }
    ...
    const claim = enqueueMaintenanceRange(
      hookInput.cwd,
      hookInput.session_id,
      messages.length - 1,
      (startIndex, throughIndex): MaintenanceQueueJob => {
        const newMessages = formatMessagesForLetta(messages, startIndex, log);
        if (newMessages.length === 0) throw new Error('maintenance enqueue reserved a range with no formatted messages');
...
The foreground sees only explicit deliver_whisper output. Ordinary assistant prose from this background pass is not a whisper and will not be injected.
        ...
        const batchId = makeBatchId(hookInput.session_id, startIndex, throughIndex);
        const canonicalMessages = buildCanonicalMessages(messages.slice(0, throughIndex + 1), startIndex, conversationId);
        const jobAssistantIntents = extractAssistantRememberIntents(
          messages.slice(0, throughIndex + 1),
          startIndex,
          hookInput.session_id,
          subjectId,
        );
        ...
        const nativePayload = {
          ...
          batchId,
          canonicalMessages,
          assistantIntents: jobAssistantIntents,
          latestUserMessage,
          ...(latestUserMessageId ? { latestUserMessageId } : {}),
          ...(foregroundRecallTurns.length ? { foregroundRecallTurns } : {}),
        };
        const job: MaintenanceQueueJob = {
          schema_version: 1,
          job_id: batchId,
          session_id: hookInput.session_id,
          start_index: startIndex,
          through_index: throughIndex,
          created_at: new Date().toISOString(),
          payload: nativePayload,
          transcript_messages: messages.slice(startIndex + 1, throughIndex + 1),
        };
        publishMaintenanceQueueJob(hookInput.cwd, job);
        log(`Published immutable maintenance queue job ${job.job_id} for indices ${startIndex + 1}..${throughIndex}`);
        return job;
      },
      log,
    ...
    spawnDrainer();
    log('Hook completed (maintenance queue drainer running in background)');
  } catch (error) {
