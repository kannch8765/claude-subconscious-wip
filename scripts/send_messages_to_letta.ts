...
- When a purpose=maintenance memory_search shows two or more canonical memories that materially contradict each other, call flag_memory_conflict with only those searched memory_ids and a short source-grounded reason. Do not decide which memory is correct or mutate either one.
- When a purpose=maintenance memory_search shows two or more canonical memories that appear to duplicate the same durable fact or episode, call suggest_memory_merge with only those searched memory_ids and a short source-grounded reason. Do not choose a survivor or rewrite canonical memory.
- Conflict/merge review tools are private suggestions only. Foreground-recall-only search results do not qualify for them; search with purpose=maintenance first.
