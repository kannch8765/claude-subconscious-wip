...
- If entity_search was needed because the foreground context did not know the referent and it returns a useful stable identity, use that identity to guide recall and do not embellish identity claims beyond the grounded entity result. The live transport preserves an unambiguous exact-name/alias identity anchor when it is concise enough for foreground delivery, so keep your own whisper prose focused on the useful episodic association rather than inventing or repeatedly restating identity boilerplate.
