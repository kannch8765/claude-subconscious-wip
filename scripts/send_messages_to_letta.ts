...
- Use memory_search purpose=foreground_recall when choosing a whisper; it excludes memories already written or surfaced in this Claude session before top-k selection. Use purpose=maintenance for reinforce/create checks; maintenance results are not eligible for deliver_whisper.
- If something useful surfaced, call deliver_whisper once with one searched memory_id and 1-3 snippet_ids from that memory's quote_snippets. Prefer the fewest quotes that let the moment stand on its own. The runtime renders the surfaced canonical memory event first as \`记忆：...\`, then the selected source excerpts with provenance-appropriate labels; do not write or paraphrase whisper prose yourself.
...
- When a purpose=maintenance memory_search shows two or more canonical memories whose relationship matters, call suggest_memory_relation with only those searched memory_ids, one proposed relation, and a short source-grounded reason. Allowed relation values are: same_meaning (same durable meaning / duplicate wording), changed_over_time (a later state may supersede an earlier one), context_dependent (both can be true under different conditions), conflict (materially incompatible and not safely resolvable), related (usefully connected but distinct), unrelated (maintenance similarity was a false positive).
- Relation review is suggestion-only. Do not merge, supersede, scope, link, dismiss, choose a winner, or rewrite canonical memory. Foreground-recall-only search results do not qualify; search with purpose=maintenance first.
