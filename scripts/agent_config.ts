...
async function ensureRequiredAgentTags(apiKey: string, agentId: string, log: (msg: string) => void = console.log): Promise<void> {
  ...
  if (!response.ok) log(`Warning: Could not update agent tags: ${response.status}`);
}
...
async function renameAgent(apiKey: string, agentId: string, name: string): Promise<void> {
  ...
  if (!response.ok) console.error(`Warning: Could not rename agent: ${response.status}`);
}
...
export function getCanonicalManagedAgentConfig(
  ...
  if (!Array.isArray(agents) || agents.length !== 1) throw new Error('Canonical Subconscious.af must contain exactly one agent');
...
async function resolveManagedModelSettingsProviderType(apiKey: string, desiredModel: string, canonical: CanonicalManagedAgentConfig): Promise<string> {
  ...
  if (!model?.provider_type) throw new Error(`Cannot resolve model_settings.provider_type for LETTA_MODEL="${desiredModel}" from Letta model metadata`);
...
export async function reconcileManagedLiveAgentSurface(
  ...
  for (const block of canonical.blocks) {
    ...
    if (existing) {
      if (LIVE_POLICY_BLOCK_LABELS.has(block.label) && existing.value !== block.value) {
        const response = await fetch(buildLettaApiUrl(`/agents/${agentId}/core-memory/blocks/${block.label}`), {
          method: 'PATCH', headers: { 'Authorization': `Bearer ${apiKey}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ value: block.value }),
        });
    ...
    const createResponse = await fetch(buildLettaApiUrl('/blocks/'), {
      ...
      body: JSON.stringify({ label: block.label, value: block.value, limit: block.limit, ...(block.description ? { description: block.description } : {}), read_only: block.readOnly }),
    });
...
export async function reconcileManagedAgentConfiguration(
  ...
  const url = buildLettaApiUrl(`/agents/${agentId}`);
  const getResponse = await fetch(url, { method: 'GET', headers: { 'Authorization': `Bearer ${apiKey}` } });
  if (!getResponse.ok) throw new Error(`Failed to read managed agent before configuration reconciliation: ${getResponse.status} ${await getResponse.text()}`);
  ...
  const parallelIsEffective = modelSettingsParallel === canonical.parallelToolCalls && legacyConfigParallel === canonical.parallelToolCalls;
  ...
  const patchResponse = await fetch(url, {
    method: 'PATCH', headers: { 'Authorization': `Bearer ${apiKey}`, 'Content-Type': 'application/json' }, body: JSON.stringify(patch),
  });
  if (!patchResponse.ok) throw new Error(`Failed to reconcile managed Subconscious runtime configuration: ${patchResponse.status} ${await patchResponse.text()}`);
  if (patch.model_settings) {
    const verifyResponse = await fetch(url, { method: 'GET', headers: { 'Authorization': `Bearer ${apiKey}` } });
    if (!verifyResponse.ok) throw new Error(`Failed to verify managed Subconscious runtime configuration after reconciliation: ${verifyResponse.status} ${await verifyResponse.text()}`);
    ...
    if (verifiedModelSettingsParallel !== canonical.parallelToolCalls || verifiedLegacyConfigParallel !== canonical.parallelToolCalls) {
      throw new Error(`Managed Subconscious effective parallel_tool_calls reconciliation failed: model_settings=${String(verifiedModelSettingsParallel)}, llm_config=${String(verifiedLegacyConfigParallel)}, expected=${String(canonical.parallelToolCalls)}`);
    }
  }
  log(`Reconciled managed Subconscious runtime configuration from ${path.basename(agentFile)}: ${Object.keys(patch).join(', ')}`);
}

async function isManagedEnvAgent(apiKey: string, agentId: string, log: (msg: string) => void = console.log): Promise<boolean> {
  const url = buildLettaApiUrl(`/agents/${agentId}`);
...
function getAgentModelHandle(agent: AgentDetails): string | null {
  ...
  if (llmConfig.provider_name && llmConfig.model) return `${llmConfig.provider_name}/${llmConfig.model}`;
...
export function findModel(models: LettaModel[], modelHandle: string): LettaModel | null {
  ...
  return models.find(m => {
    ...
    return handle === normalizedHandle || m.model?.toLowerCase() === normalizedHandle || `${m.provider_type}/${m.name}`.toLowerCase() === normalizedHandle;
  }) || null;
}

function selectBestModel(models: LettaModel[], preferences: string[]): string | null {
  for (const preferred of preferences) if (isModelAvailable(models, preferred)) return preferred;
  if (models.length > 0) {
...
async function ensureModelAvailable(
  ...
  try {
    const [models, agent] = await Promise.all([listAvailableModels(apiKey), getAgentDetails(apiKey, agentId)]);
    ...
    if (!selectedModel) throw new Error('No models available on this server. Please configure your Letta server with at least one LLM provider.');
...
export function buildLlmConfig(modelHandle: string, models: LettaModel[], currentConfig: LlmConfig | undefined): LlmConfig {
  ...
  if (envContextWindow) {
    ...
    if (!isNaN(parsed) && parsed > 0) config.context_window = parsed;
  }
  return config;
}
...
async function updateAgentModel(
  ...
  if (typeof llmConfig.context_window === 'number') {
    ...
    if (llmConfig.context_window !== currentConfig?.context_window) log(`Including context_window: ${llmConfig.context_window}`);
  }
  const response = await fetch(url, {
    method: 'PATCH', headers: { 'Authorization': `Bearer ${apiKey}`, 'Content-Type': 'application/json' }, body: JSON.stringify(body),
  });
  if (!response.ok) throw new Error(`Failed to update agent model: ${response.status} ${await response.text()}`);
...
export function buildManagedAgentImportPayload(
  ...
  let content: any;
  try { content = JSON.parse(fs.readFileSync(agentFile, 'utf8')); }
  catch (error) { throw new Error(`Failed to read managed AgentFile import payload from ${path.basename(agentFile)}: ${error}`); }
  if (!Array.isArray(content?.agents) || content.agents.length !== 1) throw new Error(`Managed ${path.basename(agentFile)} must contain exactly one agent`);
  ...
  if (bootstrapMatches !== 1) throw new Error(`Managed ${path.basename(agentFile)} must contain exactly one compiled system bootstrap prefix; found ${bootstrapMatches}`);
...
async function importDefaultAgent(apiKey: string, canonical: CanonicalManagedAgentConfig): Promise<string> {
  ...
  if (!result.agent_ids || result.agent_ids.length === 0) throw new Error('Import succeeded but no agent ID returned');
...
export async function getAgentId(apiKey: string, log: (msg: string) => void = console.log): Promise<string> {
  ...
  if (agentSource !== 'env') {
    try { await ensureRequiredAgentTags(apiKey, agentId, log); }
    catch (error) { log(`Warning: Could not ensure required tags: ${error}`); }
    ...
    try {
      ...
      if (configuredModel && config.model !== configuredModel) saveConfig({ ...config, model: configuredModel });
    } catch (error) { log(`Warning: Could not verify model availability: ${error}`); }
...
async function importAndSaveAgent(apiKey: string, log: (msg: string) => void, canonical: CanonicalManagedAgentConfig): Promise<string> {
  log('No agent configured - importing default Subconscious agent...');
  if (!fs.existsSync(DEFAULT_AGENT_FILE)) throw new Error(`Default agent file not found: ${DEFAULT_AGENT_FILE}`);
  const agentId = await importDefaultAgent(apiKey, canonical);
  log(`Imported agent: ${agentId}`);
  saveConfig({ agentId, importedAt: new Date().toISOString() });
