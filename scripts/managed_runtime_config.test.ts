...
describe('Task 093AC managed runtime configuration authority', () => {
  ...
  it('carries canonical parallel model_settings across a context-only llm_config rebuild and is idempotent', async () => {
    const canonical = getCanonicalManagedAgentConfig();
    const overrideContext = canonical.contextWindowLimit + 1024;
    process.env.LETTA_CONTEXT_WINDOW = String(overrideContext);
    const live = {
      id: ID,
      name: 'Subconscious',
      system: canonical.system,
    ...
    vi.stubGlobal('fetch', vi.fn(async (_input: string | URL | Request, init?: RequestInit) => {
      if ((init?.method ?? 'GET') === 'PATCH') {
        ...
        if (typeof patch.context_window_limit === 'number') {
          live.context_window_limit = patch.context_window_limit;
          // Letta 0.16.8 rebuilds llm_config when context changes. Model the
          // rebuild as losing parallelism unless the same PATCH carries it.
          live.llm_config = {
            ...live.llm_config,
            context_window: patch.context_window_limit,
