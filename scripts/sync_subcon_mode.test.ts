...
describe('additive synchronous Subcon mode contract', () => {
  it('keeps the existing Stop lane asynchronous and makes sync explicit opt-in', () => {
    ...
    expect(queueWorker).toContain('releaseIfStillEmpty');
  ...
  it('scopes sync whispers to the runtime-armed UserPromptSubmit without changing async/pretool delivery', () => {
    ...
    expect(syncHook).toContain('v2 never derives transcript identity from prompt text');
    expect(syncHook).toContain('partitionPendingSubconWhispersForCheckpoint(allPendingWhispers, foregroundSync.turn_id, authorizedWhisperId)');
    expect(syncHook).toContain('discardPendingSubconWhispers(staleSyncWhispers)');
    const auth = syncHook.indexOf('const authorizedWhisperId');
    const retract = syncHook.indexOf('discardPendingSubconWhispers(staleSyncWhispers)');
    const inject = syncHook.indexOf("if (mode === 'whisper')");
    expect(auth).toBeGreaterThan(-1);
    expect(retract).toBeGreaterThan(auth);
    expect(retract).toBeLessThan(inject);
    expect(fs.existsSync(path.join(process.cwd(), 'scripts/sync_recall_shadow_worker.ts'))).toBe(false);
    expect(fs.existsSync(path.join(process.cwd(), 'relationship-memory/src/rerank'))).toBe(false);
