...
import {
  configureVerifiedOmenBackfillRuntime,
  getBackfillAgentId,
  type BackfillRuntime,
} from './backfill_agent_config.js';
...
export interface RelationshipMemoryBackfillArgs {
  transcript?: string;
...
export function parseRelationshipMemoryBackfillArgs(argv: string[]): RelationshipMemoryBackfillArgs {
  const result: RelationshipMemoryBackfillArgs = { maxBatches: 1, maxRecords: 40, maxBytes: 2 * 1024 * 1024 };
...
export async function runRelationshipMemoryBackfill(
  runtime: BackfillRuntime,
  args: RelationshipMemoryBackfillArgs,
): Promise<void> {
  ...
  const agentId = await getBackfillAgentId(apiKey, () => {}, { agentId: args.agentId, runtime });
  if (runtime === 'omen') {
...
export async function runRelationshipMemoryBackfillCli(runtime: BackfillRuntime, argv = process.argv.slice(2)): Promise<void> {
  return runRelationshipMemoryBackfill(runtime, parseRelationshipMemoryBackfillArgs(argv));
}
