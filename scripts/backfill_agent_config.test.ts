...
describe('dedicated historical backfill agent resolver', () => {
  it('uses the dedicated override and never falls through to live LETTA_AGENT_ID', async () => {
    ...
    process.env.LETTA_BACKFILL_AGENT_ID = BACKFILL;
  ...
  it('fails closed before any mutation if dedicated and live identities collapse', async () => {
    process.env.LETTA_AGENT_ID = LIVE;
    process.env.LETTA_BACKFILL_AGENT_ID = LIVE;
  ...
  it('reconciles the dedicated agent from canonical Subconscious.af and adds durable purpose identity', async () => {
    process.env.LETTA_AGENT_ID = LIVE;
    process.env.LETTA_BACKFILL_AGENT_ID = BACKFILL;
    const canonical = getCanonicalManagedSystemPrompt();
    let tags: string[] = ['git-memory-enabled', 'origin:claude-subconcious'];
    let system = 'stale prompt';
    ...
    expect(system).toBe(canonical);
    expect(patches.some((body) => body.system === canonical)).toBe(true);
  });

  it('provisions once, saves a separate backfill config, and reuses it without scanning historical agents', async () => {
    process.env.LETTA_AGENT_ID = LIVE;
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'backfill-agent-'));
    const configFile = path.join(dir, 'backfill.json');
    process.env.LETTA_BACKFILL_CONFIG_FILE = configFile;
    const canonical = getCanonicalManagedSystemPrompt();
    let tags: string[] = [];
    let agentSystem = canonical;
    let imports = 0;
    const urls: string[] = [];
