...
describe('Letta execution dependency contract', () => {
  it('pins the native API client used by backfill and realtime relationship observer lanes', () => {
    ...
    // The Code SDK remains pinned only for the separate explicit recall runtime.
    // Live/background Subconscious execution and semantic observers must stay on the native client.
  ...
  it('keeps both semantic observer execution paths off the Letta Code subprocess SDK', () => {
    ...
    const liveWorker = fs.readFileSync(path.resolve('scripts/send_worker_native.ts'), 'utf8');
    ...
    for (const source of [legacyRunner, realtimeRunner, liveWorker]) {
    ...
    expect(liveWorker).toContain('runNativeClientToolConversation');
    expect(liveWorker).toContain('createNativeLettaClient');
