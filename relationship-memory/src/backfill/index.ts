...
function observerEnvelope(file: string, generation: number, records: TranscriptMessage[], canonical: CanonicalMessage[]): string {
  ...
  return `<relationship_memory_historical_backfill>\n<source>${path.basename(file)}</source>\n<generation>${generation}</generation>\n<processable_evidence_count>${canonical.length}</processable_evidence_count>\n<transcript>\n${entries}\n</transcript>\n<instructions>Process this bounded historical passage using the same relationship-memory observer semantics as live observation. Search existing canonical memory before choosing an action. Reinforce the same underlying episode/event with memory_reinforce; create genuinely distinct episodes with memory_remember and linked_memory_ids when related. Lexical similarity alone is not sameness. Only evidence IDs from the trusted relationship-memory evidence catalog may justify either operation.</instructions>\n</relationship_memory_historical_backfill>`;
}
