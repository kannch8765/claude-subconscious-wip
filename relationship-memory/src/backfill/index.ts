...
function observerEnvelope(file: string, generation: number, records: TranscriptMessage[], canonical: CanonicalMessage[]): string {
  ...
  return `<relationship_memory_historical_backfill>\n<source>${path.basename(file)}</source>\n<generation>${generation}</generation>\n<processable_evidence_count>${canonical.length}</processable_evidence_count>\n<transcript>\n${entries}\n</transcript>\n<instructions>Process this bounded historical passage using the same relationship-memory observer semantics as live observation. Search existing canonical memory before choosing an action. Reinforce the same underlying episode/event with memory_reinforce; create genuinely distinct episodes with the matching kind-specific memory creation tool and linked_memory_ids when related. Lexical similarity alone is not sameness. New memory prose is a source-grounded historical event/stable-fact index: do not manufacture feelings, motives, fulfillment framing, or relationship conclusions beyond trusted evidence. Treat assistant self-reports such as “I do not remember”, “I have not heard of this”, or “this is new to me” as session-local epistemic state, not proof of a historical first occurrence. If memory_search finds earlier canonical evidence for the same person, project, episode, or relationship fact, preserve that chronology and do not claim “first learned”, “first discovered”, “first met”, or equivalent historical-first wording unless trusted evidence itself establishes it. Only evidence IDs from the trusted relationship-memory evidence catalog may justify either operation.</instructions>\n</relationship_memory_historical_backfill>`;
}
