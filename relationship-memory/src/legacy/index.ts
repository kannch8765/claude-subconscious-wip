...
Relationship-memory relevance is required: do not preserve ordinary technical implementation, bug-fix, deployment, configuration, monitoring, or operational-status facts merely because they are detailed, important, or historically true. If such a source has no durable user preference, identity/role significance, shared or personal experience with enduring human meaning, relationship event/change, or inside-joke/shared-language value, choose no canonical memory required. Technical context may be retained only as context for an otherwise relationship-relevant semantic item. Importance or technical detail alone is never sufficient.
Every canonical outcome must remain traceable to the immutable legacy_source_id.
Preserve actor/action fidelity literally. Being an assistant-authored legacy source does not imply that the assistant performed an action whose actor is unstated. If the source does not name an actor, use neutral/actorless wording rather than assigning the action to user, assistant, or a named person. Preserve explicit actor labels such as Sol, Sonnet, 晴, ゆう, Claude, GPT, or other names as written unless this source itself explicitly establishes an identity mapping. Never silently substitute one actor for another. Preserve the action itself just as strictly: do not strengthen, invent, or infer an action that the source does not state. For example, source wording like "X 管理的/拥有的/位于" does not authorize "X 建立/创建/迁移/提交/实现"; keep the weaker source-faithful action or use neutral wording. Actorless/passive status wording must remain actorless/passive: phrases such as "项目完成/代码已写/测试通过/列为候选" do not authorize "用户完成/琥珀完成/克宝完成/琥珀提出". Only an actor explicitly present in the source (including an explicit first-person pronoun in this legacy assistant-authored source) may be rendered as performing that action.
