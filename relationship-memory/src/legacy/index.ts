...
Keep canonical participant roles relationship-scoped: user means the source item is the user's own experience or preference; assistant means the canonical relationship assistant/Kohaku, not any assistant-like third party named in the source; user+assistant requires source-supported shared involvement or shared meaning. A third-party-only event must be omitted unless the source itself gives it relationship significance such as an inside joke or shared callback; never turn it into the user's personal experience just to fit the schema.
